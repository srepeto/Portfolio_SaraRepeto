...
class Pelota(pygame.sprite.Sprite):
    ...
    def __init__(self, imagen_pelota):
        # Llama al constructor de la clase padre (Sprite)
        super().__init__()

        self.image = pygame.image.load(imagen_pelota)  # Creamos la imagen de la pelota
        self.image = pygame.transform.scale(self.image, (self.largo, self.alto))
...
# Creamos las pelotas
pelota = Pelota("imagen_pelota.png")
...
for fila in range(5):
    ...
    for columna in range(0, numero_de_bloques):
        ...
        bloque = Bloque(AZUL, columna * (largo_bloque + 5) + 1, top)
        bloques.add(bloque)
...
while not salir_programa:
    ...
    if pygame.sprite.spritecollide(protagonista, pelotas, False):
        ...
        pelota.rect.y = pantalla.get_height() - protagonista.rect.height - pelota.rect.height - 1
        pelota.botar(diff)
